...
export default async function CategoryShowPage({ params, searchParams }: CategoryShowPageProps) {
  const { products } = await getData({
    categoryId: params.id,
    productPaginationOptions: {
      current: searchParams.current ? Number(searchParams.current as string) : 1,
    },
  });
